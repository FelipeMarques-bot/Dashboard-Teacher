...
4. **Não crie e não defina** a variável `DATABASE_URL` em `.env.local` nem `.env.api`; é justamente a ausência dela que ativa o armazenamento local em arquivo (`server/data/state.json`).
